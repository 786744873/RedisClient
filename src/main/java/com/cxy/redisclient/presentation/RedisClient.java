...
public class RedisClient {
	...
	private ZSetService zSetService = new ZSetService();
	...
	private void initShell() {
		...
		shell.setText("RedisClient （用爱发电版）");
	...
	private void serverTreeItemSelected(TreeItem selectedItem, boolean refresh) {
		...
		Map<String, String[]> dbInfoMap = serverService.listInfo((Integer) selectedItem.getData(NODE_ID));
		String[] keySpaces = dbInfoMap.get(" Keyspace");
		Map<Integer,Long> dbSizeMap=new HashMap();
		Arrays.stream(keySpaces).forEach(keySpace->{
			String[] dbInfo = keySpace.split(",")[0].split(":");
			int db = Integer.parseInt(dbInfo[0].replace("db", ""));
			long dbSize = Long.parseLong(dbInfo[1].replace("keys=", ""));
			dbSizeMap.put(db,dbSize);
		});
		...
		if (selectedItem.getData(ITEM_OPENED) == null
			...
			for (int i = 0; i < amount; i++) {
				...
				dbItem.setText(DB_PREFIX + i+getDbSizeStr(dbSizeMap,i));
		...
		for (int i = 0; i < amount; i++) {
			TableItem item = new TableItem(table, SWT.NONE);
			item.setText(new String[] { DB_PREFIX + i+getDbSizeStr(dbSizeMap,i),
	...
	private String getDbSizeStr(Map<Integer,Long> dbSizeMap,int i)
	{
		return dbSizeMap.get(i)==null?" (0)":" ("+dbSizeMap.get(i)+")";
	}
